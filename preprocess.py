...
from scipy.io import savemat # test
...
def get_fc_graph(fc_folder, sc_folder, list_file):
    """
    ...
    target_subject_prefixes = pd.read_csv(list_file)['subject_file'].values
    fc_list, sc_list = [], []

    for prefix in target_subject_prefixes:
        fc_file_path = None
        sc_file_path = None
        if 'fcMRI' in prefix:
            fc_file_path = f"{fc_folder}/{prefix}_connectivity_matrix_file.mat"
            fc = scipy.io.loadmat(fc_file_path)['data']

            # FC normalization
            fc = np.log((1 + fc) / (1 - fc))  # fisher transform
            fc = normalization(fc)  # normalization
            # fc = normalization(fc + np.eye(188))  # normalization

            fc_list.append(fc)
        else:
            sc_file_path = f"{sc_folder}/{prefix}_connectivity_matrix_file.mat"
            sc = scipy.io.loadmat(sc_file_path)['data']
...
if __name__ == '__main__':
    ...
    label_path = './dataset/raw_data/control_subject_label.csv'
